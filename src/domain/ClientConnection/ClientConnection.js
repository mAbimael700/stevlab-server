...
const { FTPMonitor } = require("../../lib/connections/FTP/FtpMonitor.js");
...
class ClientConnection {
  ...
  async #setupTcpServer(equipmentRepository) {
    const tcpClient = new TCPClient(this.equipment, equipmentRepository);
    this.client = await tcpClient.build();
    this.connect = tcpClient.connect
    this.reconnect = tcpClient.reconnect
  ...
  async #setupFtp() {
    ...
    this.connect = ftpClient.build
    this.reconnect = ftpClient.reconnect
    this.#updateState(this.client);
    this.monitor = new FTPMonitor(this.equipment)
  }
