...
class Hl7Segment {
    constructor(name, fields) {
        this.name = name;
        this.fields = fields.map(field => field?.replace(/[\n\r\x1C]/g, '') || ''); // Limpiar caracteres de control
    ...
    getAllFieldsWithNames() {
        ...
        this.fields.forEach((value, index) => {
            if (index > 0 && value) { // Omitir el índice 0 que es el nombre del segmento
                const fieldName = Hl7FieldDefinitions.getFieldDefinition(this.name, index);
