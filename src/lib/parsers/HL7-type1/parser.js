...
function parser(parsedData) {
  const results = [];
  const formatString = "dd-MM-yyyyHH:mm";

  parsedData.forEach((section) => {
    section.forEach((line) => {
      if (line[0] === "R") {
        // Parsea un bloque de resultados (líneas que comienzan con 'R')
        const dateString = line[2].trim() + line[3].trim();
        // Parseando la fecha a un objeto Date
        ...
        for (let i = 12; i < line.length; i += 7) {
          if (line[i]) {
            ...
            const parametro = {
              ...
              rango_min: parseFloat(line[i + 4]?.trim()),
              rango_max: parseFloat(line[i + 5]?.trim()),
